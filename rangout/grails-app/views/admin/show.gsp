	...
	<body>
		...
		<div id="show-admin" class="content scaffold-show" role="main">
			...
			<ol class="property-list admin">
				...
				<g:if test="${adminInstance?.restaurant}">
				<li class="fieldcontain">
					<span id="restaurant-label" class="property-label"><g:message code="admin.restaurant.label" default="Restaurant" /></span>
					
						<span class="property-value" aria-labelledby="restaurant-label"><g:link controller="restaurant" action="show" id="${adminInstance?.restaurant?.id}">${adminInstance?.restaurant?.encodeAsHTML()}</g:link></span>
